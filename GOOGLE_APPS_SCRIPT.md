...
function doPost(e) {
  try {
    ...
    if (action === "createRecipe") {
      ...
      const result = updateRecipe(data);
      return createResponse({
        status: "success",
      ...
      const result = deleteRecipe(data.id);
      return createResponse({
        status: "success",
...
function updateRecipe(data) {
  ...
  const recipeId = parseInt(data.id);

  // recipeId viene del loop (i=1 significa la 2da fila, i=2 significa la 3ra fila, etc)
  // Entonces la fila en Google Sheets = recipeId + 1
  const targetRow = recipeId + 1;

  if (targetRow < 2 || targetRow > sheet.getLastRow()) {
    throw new Error(
      `Receta no encontrada. ID: ${recipeId}, Fila: ${targetRow}, Última: ${sheet.getLastRow()}`
    );
  }
  ...
  sheet.getRange(targetRow, 3).setValue(data.ingredients);
  sheet.getRange(targetRow, 4).setValue(data.instructions);
  sheet.getRange(targetRow, 5).setValue(new Date().toLocaleString("es-MX"));
...
function deleteRecipe(recipeId) {
  ...
  if (targetRow < 2 || targetRow > sheet.getLastRow()) {
    throw new Error(`Receta no encontrada. ID: ${id}, Fila: ${targetRow}`);
  }
